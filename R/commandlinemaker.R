commandlinemaker <- function(model, partition, data) {
    ...
    if (model$pInv) {
        commandline <- cbind(commandline, paste("-i", data$log[, paste(partition, 
            "pInv", sep = ".")], sep = ""))
    ...
    if (model$basefreq == "estimated") {
        freqs <- cbind(data$log[, paste(partition, "frequencies1", sep = ".")], data$log[, 
            paste(partition, "frequencies2", sep = ".")], data$log[, paste(partition, 
            "frequencies3", sep = ".")], data$log[, paste(partition, "frequencies4", 
            sep = ".")])
        p.freqs <- freqs
    ...
    if (model$rmat == "gtr") {
        browser()
        rmatrix <- cbind(data$log[, paste(partition, "ac", sep = ".")], data$log[, 
            paste(partition, "ag", sep = ".")], data$log[, paste(partition, "at", 
            sep = ".")], data$log[, paste(partition, "cg", sep = ".")], 1, data$log[, 
            paste(partition, "gt", sep = ".")])
        p.rmat <- rmatrix
        rmatrix <- paste(rmatrix[, 1], rmatrix[, 2], rmatrix[, 3], rmatrix[, 4], 
            rmatrix[, 5], rmatrix[, 6], sep = ",")
        rmatrix2 <- paste("-r", rmatrix, sep = "")
        commandline <- cbind(commandline, rmatrix2)
    }
    if (model$rmat == "hky") {
        rmatrix <- cbind(1, data$log[, paste(partition, "kappa", sep = ".")], 1, 
            1, data$log[, paste(partition, "kappa", sep = ".")], 1)
        p.rmat <- rmatrix
        rmatrix <- paste(rmatrix[, 1], rmatrix[, 2], rmatrix[, 3], rmatrix[, 4], 
            rmatrix[, 5], rmatrix[, 6], sep = ",")
        rmatrix2 <- paste("-r", rmatrix, sep = "")
    ...
    if (model$rmat == "trn") {
        rmatrix <- cbind(1, data$log[, paste(partition, "kappa1", sep = ".")], 1, 
            1, data$log[, paste(partition, "kappa2", sep = ".")], 1)
        p.rmat <- rmatrix
        rmatrix <- paste(rmatrix[, 1], rmatrix[, 2], rmatrix[, 3], rmatrix[, 4], 
            rmatrix[, 5], rmatrix[, 6], sep = ",")
        rmatrix2 <- paste("-r", rmatrix, sep = "")
    ...
    out <- list(seqgen = commandline, phangorn = list(p.rmat = p.rmat, p.freqs = p.freqs, 
        p.inv = p.inv, p.alpha = p.alpha, kg = kg))
    return(out)
} 
